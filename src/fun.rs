use crate::lex::{VarType, VarVal, process_template_value};
use crate::log::Log;
...
use std::{
    ...
    fmt,
    fs::{
        self, File, OpenOptions, copy, create_dir_all, remove_dir, remove_dir_all, remove_file,
        rename,
    },
    io::{self, Write},
    ops::Deref,
...
impl GenBlockTup {
    ...
    pub fn remove_var(&self, name: &str) -> Option<VarVal> {
    ...
    fn search_up_block_type(&self, block_type: BlockType) -> Option<GenBlockTup> {
        ...
        loop {
            let current_bare = current_bl.borrow();
            // println!{"checking {:?} for {name}", parent_bare.block_type}
            if current_bare.block_type == block_type {
                break Some(current_bl.clone());
            } else {
                let parent = current_bare.parent.clone()?;
                drop(current_bare);
            ...
            match dep_block.block_type {
                    ...
                    match dep_block.name.clone().unwrap().as_str() {
                        "target" => {
                            ...
                            match target {
                                ...
                                _ => log.warning(&format!(
                                    "Target {} not found and ignored",
                                    dep_block.params[0]
                                )),
                            }
                        }
                        "anynewer" => {
                            ...
                            let p2 =
                                process_template_value(log, &dep_block.params[1], &dep, prev_res);
                ...
                BlockType::Eq => {
                    ...
                    if len > 0 {
                        ...
                        let r1: Option<VarVal> = match p1_block.block_type {
                            BlockType::Function => p1.exec_fun(log, &p1_block, prev_res),
                            _ => {
                                todo!("block: {:?}", p1_block.block_type)
                            }
                        };
                        let r2: Option<VarVal> = if len == 2 {
                            let p2 = &dep_block.children[1];
                            let p2_block = p2.0.borrow();
                            match p2_block.block_type {
                                BlockType::Function => p2.exec_fun(log, &p2_block, prev_res),
                                _ => {
                                    todo!("block: {:?}", p2_block.block_type);
                                }
                            }
                        } else {
                            None
                ...
                _ => todo!(
                    "the operation {:?} isn't supported yet at {}:{}: ",
                    dep_block.block_type,
                    dep.script_path(),
                    dep.script_line
                ),
            }
    ...
    pub fn get_target(&self, name: &String) -> Option<GenBlockTup> {
        ...
        for ch in &naked_block.children {
            ...
            if ch_block.block_type == BlockType::Target
                && let Some(blk_name) = &ch_block.name
                && blk_name == name
            {
        ...
        match block_type {
            ...
            BlockType::For => {
                ...
                let Some(name) = &naked_block.name.clone() else {
                    log.error(&format!(
                        "A 'for' variable isn't specified at {}:{}: ",
                        naked_block.script_path(),
                        naked_block.script_line
                    ));
                    return None;
                ...
                let Some(range_as_opt) = &naked_block.dir.clone() else {
                    log.error(&format!(
                        "A 'for' range isn't specified at {}:{}: ",
                        naked_block.script_path(),
                        naked_block.script_line
                    ));
                    return None;
                ...
                if let Some(range_as_val) = range_as_var {
                    if range_as_val.val_type == VarType::Array {
                        ...
                        let Some(sep_can) = &naked_block.flex.clone() else {
                            log.error(&format!(
                                "A 'for' values separator isn't specified at {}:{}: ",
                                naked_block.script_path(),
                                naked_block.script_line
                            ));
                            return None;
                        ...
                        let range_as_val = process_template_value(
                            log,
                            &range_as_val.value,
                            &naked_block,
                            prev_res,
            ...
            BlockType::Eq => {
                ...
                for child in children.iter().take(len).skip(1) {
                    ...
                    match res {
                        None => {
                            if let Some(_before_some) = before_res {
                                return Some(VarVal::from_bool(false));
                            }
                        }
                        Some(ref res_some) => match before_res {
                            None => return Some(VarVal::from_bool(false)),
                            Some(ref before_some) => {
                                if before_some.value != res_some.value {
                                    return Some(VarVal::from_bool(false));
                                }
                            }
                        },
            ...
            _ => {
                let naked_block = self.borrow();
                todo!(
                    "not implemented block: {:?}, {:?} at {}:{}: ",
                    naked_block.block_type,
                    &naked_block.name,
                    naked_block.script_path(),
                    &naked_block.script_line
                )
    ...
    pub fn exec_fun(
        ...
        match name {
            ...
            "exec" | "aexec" => {
                    ...
                    if let Some(param) = val {
                        if !param.values.is_empty() {
                            ...
                            for param in param.values {
                                params
                                    .push(*process_template_value(log, &param, fun_block, res_prev))
                            }
                        } else if param.val_type != VarType::Array {
                            params.push(*process_template_value(
                                log,
                                &param.value,
                                fun_block,
                                res_prev,
                ...
                let mut calc_cwd = |work_dir_val: &String| {
                    if !work_dir_val.is_empty() {
                        let mut work_dir = match fun_block.search_up(work_dir_val) {
                            Some(work_dir_val1) => *process_template_value(
                                log,
                                &work_dir_val1.value,
                                fun_block,
                                res_prev,
                ...
                if let Some(_dry_run) = dry_run {
                    ...
                    match status {
                        Ok(status) => match status.code() {
                            Some(code) => return Some(VarVal::from_i32(code)),
            ...
            "scalar" | "join" => {
                // vector var, separator
                let sep = if fun_block.params.len() > 1 {
                    *self.parameter(log, 1, fun_block, res_prev)
                } else {
                    "\t".to_string()
                };
                let var = &self.array_to_string(
                ...
                if no_parameters(fun_block) {
                    ...
                    if fun_block.params.len() == 2 {
                        ...
                        let time = if time_str == "now" {
                            ...
                            UNIX_EPOCH
                                + Duration::from_secs(
                                    time::seconds_from_epoch(1970, y, m, d, h, mn, s)
                                        .unwrap_or_default(),
                                )
                        } else {
                            SystemTime::now()
            ...
            "cropname" => {
                // the approach targets a file path, so for arbitrary name add a CWD to mask
                let mut fname = *self.parameter(log, 0, fun_block, res_prev);
                ...
                if crop_end {
                    ...
                    if fname.ends_with(&mask) {
                        if fun_block.params.len() == 3 {
                            ...
                            return Some(VarVal::from_string(
                                fname[0..fname.len() - mask.len()].to_owned() + &subs,
                            ));
                        }
                    ...
                    if fname.starts_with(&mask)
                        && (end_mask.is_empty() || fname.ends_with(&end_mask))
                    {
                        if fun_block.params.len() == 3 {
                            let subs = *self.parameter(log, 2, fun_block, res_prev);
                            ...
                            if subs.len() == 1 {
                                return Some(VarVal::from_string(
                                    subs[0].to_owned() + &fname[mask.len()..],
                                ));
                            } else {
                                return Some(VarVal::from_string(
                                    subs[0].to_owned()
                                        + &fname[mask.len()..(fname.len() - end_mask.len())]
                                        + subs[1],
                                ));
                            }
                        }
            ...
            "files" => {
                ...
                for i in 0..fun_block.params.len() {
                    ...
                    if !has_root(&file)
                        && let Some(ref path) = cwd
                    {
                        file = path.value.clone() + MAIN_SEPARATOR_STR + &file
                    }
            ...
            "rmdir" | "rmdira" => {
                ...
                for i in 0..fun_block.params.len() {
                    ...
                    if !file.is_empty() {
                        ...
                        if name == "rmdira" && remove_dir_all(&file).is_ok()
                            || remove_dir(&file).is_ok()
                        {
                            res.push(file)
                        }
                    }
            ...
            "zip" => {
                ...
                {
                    zip_path = cwd.value.clone() + MAIN_SEPARATOR_STR + &zip_path
                }
                ...
                while current_op < flatten_params.len() {
                    let op = *self.expand_parameter(
                        log,
                        &flatten_params[current_op],
                        fun_block,
                        res_prev,
                    );
                    ...
                    if op.starts_with("-A") || op.starts_with("-E") {
                        ...
                        let cont = *self.expand_parameter(
                            log,
                            &flatten_params[current_op],
                            fun_block,
                            res_prev,
                        ...
                        if !has_root(&files)
                            && let Some(ref cwd) = cwd
                        {
                            files = cwd.value.clone() + MAIN_SEPARATOR_STR + &files
                        }
                        ...
                        //
                        let zipped_path =
                            self.prev_or_search_up(&flatten_params[current_op], res_prev);
                        //println!{"found pd {zipped_path:?} for {}", &flatten_params[current_op]}
                        // nested to nested case?
                        let values = if let Some(zipped_path) = zipped_path {
                        ...
                        for mut entry in values {
                            ...
                            if !has_root(&entry)
                                && let Some(ref cwd) = cwd
                            {
                                entry = cwd.value.clone() + MAIN_SEPARATOR_STR + &entry
                            }
                            ...
                            if let Some((before, after)) = filename.split_once("*") {
                                ...
                                match parent_files.read_dir() {
                                    ...
                                    _ => log.warning(
                                        &format! {"Zip: can't process directory {parent_files:?}"},
                                    ),
                                }
                                ...
                                match entry_path.read_dir() {
                                    ...
                                    _ => log.warning(
                                        &format! {"Zip: can't read directory{entry_path:?}"},
                                    ),
                                }
                            } else {
            ...
            fun @ "to_upper" => {
                if fun_block.params.len() == 1 {
                    return Some(VarVal::from_string(
                        self.parameter(log, 0, fun_block, res_prev).to_uppercase(),
                    ));
                } else {
                    log.error(&format!(
            ...
            fun @ "to_capitalized" => {
                if fun_block.params.len() == 1 {
                    return Some(VarVal::from_string(util::capitalize_first(
                        &self.parameter(log, 0, fun_block, res_prev),
                    )));
                } else {
                    log.error(&format!(
    ...
    pub fn exec_assign(
        ...
        loop {
            ...
            if close_scope.is_none()
                && (parent_bare.block_type == BlockType::Scope
                    || parent_bare.block_type == BlockType::Target
        ...
        match param {
            ...
            Some(val) => {
                let var = match val.val_type {
                    VarType::Environment => match env::var(&val.value) {
                        Ok(val) => val,
                        Err(_e) => val.value,
                    },
                    VarType::Property => {
        ...
        if vec_param.val_type == VarType::Array {
            Some(
                vec_param
                    ...
                    .into_iter()
                    .map(|v| {
                        if let Some(v) = self.prev_or_search_up(&v, res_prev) {
                            self.array_to_string(&Some(v), sep, res_prev)
                                .unwrap_or_default()
                        } else {
                            v
                        }
            ...
            match c {
                ' ' | '\t' | '\n' | '\r' => {
                    match state {
                        ...
                        CalcState::Exp => prev_state = state,
                        ...
                        CalcState::Oper => state = CalcState::Start,
                        CalcState::Skip => {
                            buf_var[name_pos] = c;
                            name_pos += 1
                        }
                ...
                '*' | '/' => {
                    ...
                    match state {
                        CalcState::Start => return Err((CalcErrCause::InvOp, pos)),
                        CalcState::Var => {
...
pub fn run(log: &Log, block: GenBlockTup, targets: &mut Vec<String>) -> Result<(), Box<dyn Error>> {
    ...
    'targets: for target in targets {
        ...
        for bl in children {
            ...
            if ch_block.block_type == BlockType::Target
                && ch_block.name.as_ref().ok_or("no 'target' block name")? == target
            {
                drop(ch_block);
                log.log(&format!(
                    "target: {}",
                    exec_target(log, &bl, bl.search_up("~build-given-target~").is_some())
                ));
                continue 'targets;
            }
        }
    ...
    if let Some(dir) = dir {
        ...
        if !dir_val.is_empty() {
            ...
            if path.exists() {
                ...
                target
                    .vars
                    .insert(String::from(CWD), VarVal::from_string(cwd));
            } else {
